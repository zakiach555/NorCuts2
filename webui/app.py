...
def run_viral_cutter(input_source, project_name, url, video_file, segments, viral, themes, min_duration, max_duration, model, ai_backend, api_key, ai_model_name, chunk_size, workflow, face_model, face_mode, face_detect_interval, no_face_mode,
                     ...
                     use_custom_subs, preset_name, font_name, font_size, font_color, highlight_color, outline_color, outline_thickness, shadow_color, shadow_size, is_bold, is_italic, is_uppercase, vertical_pos, alignment,
                     ...
                     spk_name_size=0, spk_title_size=0):
    ...
    if use_custom_subs:
        # If a named preset is selected, use its server-side values so stale
        # browser ColorPicker state can't override what the user chose.
        if preset_name and preset_name != "Manual" and preset_name in subs.SUBTITLE_PRESETS:
            p = subs.SUBTITLE_PRESETS[preset_name]
            font_color      = p.get("base_color",      font_color)
            highlight_color = p.get("highlight_color", highlight_color)
            outline_color   = p.get("outline_color",   outline_color)
            shadow_color    = p.get("shadow_color",    shadow_color)
        ...
        subtitle_config["watermark_text"]  = watermark_text or ''
        subtitle_config["speaker_name"]    = speaker_name or ''
        subtitle_config["speaker_title"]   = speaker_title or ''
        subtitle_config["wm_transparency"] = int(wm_transparency) if wm_transparency is not None else 25
        subtitle_config["wm_outline"]      = wm_outline if wm_outline is not None else 0
        subtitle_config["wm_shadow"]       = wm_shadow if wm_shadow is not None else 1
        subtitle_config["spk_name_size"]   = int(spk_name_size) if spk_name_size else 0
        subtitle_config["spk_title_size"]  = int(spk_title_size) if spk_title_size else 0
        subtitle_config["lang"] = translate_target if translate_target and translate_target != "None" else 'ar'
...
with gr.Blocks(title=i18n("ViralCutter WebUI"), theme=gr.themes.Default(primary_hue="orange", neutral_hue="slate"), css=css) as demo:
    ...
    with gr.Tabs():
        with gr.Tab(i18n("Create New")):
             with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        translate_input = gr.Dropdown(choices=["None", "ar", "en"], label=i18n("Translate Subtitles To"), value="ar")
                    ...
                    with gr.Row():
                        segments_input = gr.Number(label=i18n("Segments"), value=1, precision=0)
                ...
                with gr.Column(scale=1):
                    ...
                    model_input = gr.Dropdown(["tiny", "small", "medium", "large", "large-v1", "large-v2", "large-v3", "turbo", "large-v3-turbo", "distil-large-v2", "distil-medium.en", "distil-small.en", "distil-large-v3"], label=i18n("Whisper Model"), value="tiny")
                    with gr.Row():
                    ...
                    with gr.Row():
                        ...
                        no_face_mode_input = gr.Dropdown(choices=[(i18n("Padding (9:16)"), "padding"), (i18n("Zoom (Center)"), "zoom")], label=i18n("No Face Fallback"), value="padding")
             ...
             with gr.Accordion(i18n("Subtitle Settings (alpha)"), open=False):
                preset_input = gr.Dropdown(choices=[(i18n("Manual"), "Manual")] + [(i18n(k), k) for k in subs.SUBTITLE_PRESETS.keys()], label=i18n("Quick Presets"), value="Arabic Two-Line Emphasis (RTL)")
                ...
                manual_inputs = [
                    ...
                    remove_punc_input,
                    watermark_text_input, speaker_name_input, speaker_title_input,
                    wm_transparency_input, wm_outline_input, wm_shadow_input,
                    spk_name_size_input, spk_title_size_input,
                    translate_input,
                ]
            ...
            def render_single(json_path, use_custom, font_name, font_size, font_color, highlight_color,
                ...
                if use_custom:
                    subtitle_config = {
                        ...
                        "wm_transparency": int(wm_transparency) if wm_transparency is not None else 25,
                        "wm_outline": wm_outline if wm_outline is not None else 0,
                        "wm_shadow": wm_shadow if wm_shadow is not None else 1,
                        "spk_name_size": int(spk_name_size) if spk_name_size else 0,
                        "spk_title_size": int(spk_title_size) if spk_title_size else 0,
                    }
                    try:
                        with open(subtitle_config_path, "w", encoding="utf-8") as f:
                            json.dump(subtitle_config, f, indent=4)
    ...
    gr.Markdown("""
        ...
        <div style='text-align: center; font-size: 0.9em; color: #777;'>
            <p>100% local &bull; open source &bull; no subscription required</p>
        </div>
        """)
if __name__ == "__main__":
    ...
    if args.colab:
        ...
        if is_windows:
            ...
            app, local_url, share_url = demo.queue(default_concurrency_limit=None).launch(
                share=False,
                allowed_paths=allowed_dirs,
                inbrowser=True,
                server_name="0.0.0.0",
