...
def main():
    ...
    parser.add_argument("--ai-backend", choices=["manual", "gemini", "openrouter", "openai", "deepseek", "zhipu", "qwen", "huggingface", "g4f", "local"], help="AI backend for viral analysis")
    parser.add_argument("--api-key", help="API key for the selected AI backend")
    ...
    try:
        ...
        if burn_subtitles_option:
            ...
            if args.translate_target and args.translate_target.lower() != "none":
                 ...
                 import asyncio
                 try:
                    _trans_api_key  = getattr(args, 'api_key', None)
                    _trans_ai_model = getattr(args, 'ai_model_name', None)
                    asyncio.run(translate_json.translate_project_subs(
                        project_folder, args.translate_target,
                        api_key=_trans_api_key, ai_model=_trans_ai_model,
                    ))
                 except Exception as e:
            ...
            try:
                subtitle_lang = args.translate_target if args.translate_target and args.translate_target.lower() != "none" else 'en'
                sub_config.pop('lang', None)  # avoid duplicate-keyword if lang was saved in config JSON
                adjust_subtitles.adjust(project_folder=project_folder, lang=subtitle_lang, **sub_config)
