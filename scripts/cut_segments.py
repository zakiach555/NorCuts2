...
def cut(segments, project_folder="tmp", skip_video=False):
    ...
    def detect_best_encoder():
        test_cmd_base = ["ffmpeg", "-f", "lavfi", "-i", "nullsrc=s=64x64:d=0.1", "-frames:v", "1"]
        for encoder, label in [("h264_nvenc", "NVENC"), ("h264_qsv", "QSV"), ("h264_mf", "MediaFoundation")]:
            try:
                ...
                if result.returncode == 0:
                    print(f"Using hardware encoder: {encoder} ({label})")
                    return encoder
            except Exception:
                pass
        print("No hardware encoder available. Using libx264.")
        return "libx264"

    def generate_segments(response, project_folder, skip_video):
        video_codec = detect_best_encoder()
        ...
        for i, segment in enumerate(segments):
            ...
            if not skip_video:
                ...
                if video_codec == "h264_nvenc":
                    ...
                    command.extend(["-preset", "veryfast", "-b:v", "5M"])
                elif video_codec == "h264_mf":
                    command.extend(["-b:v", "5M"])
                else:
