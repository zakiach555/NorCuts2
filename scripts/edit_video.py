...
def get_best_encoder():
    ...
    candidates = [
        ("h264_nvenc", "fast", "NVIDIA"),
        ("h264_amf", "speed", "AMD"),
        ("h264_qsv", "veryfast", "Intel QSV"),
        ("h264_videotoolbox", "default", "macOS"),
        ("h264_mf", "default", "MediaFoundation"),
    ]
    for encoder, preset, label in candidates:
        try:
            r = subprocess.run(
                ["ffmpeg", "-f", "lavfi", "-i", "nullsrc=s=64x64:d=0.1", "-frames:v", "1", "-c:v", encoder, "-f", "null", "-"],
...
def generate_short_mediapipe(input_file, output_file, index, face_mode, project_folder, final_folder, face_detection, face_mesh, pose, detection_period=None, no_face_mode="padding"):
    try:
        ...
        if detection_period is not None and not isinstance(detection_period, dict):
